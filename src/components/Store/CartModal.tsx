...
import PaymentModal from './PaymentModal'; // Assuming correct path
...
const CartModal: React.FC<CartModalProps> = ({ 
  ...
  const total = subtotal + tax; // ✅ Calculated total here
  console.log("CartModal: calculated total =", total);
  const handleCheckout = () => {
    // ⚠️ CRITICAL CHECK: Prevent checkout if the total amount is zero
    if (total <= 0) {
      alert("Cannot proceed to checkout with a total amount of $0.00.");
      return; 
    }
    
    console.log('CartModal: checkout clicked, opening payment modal');
    ...
    try {
      ...
      console.error('CartModal: onCheckout threw:', err);
    }
  };

  const handlePaymentSuccess = (paymentData: any) => {
    console.log('Payment successful:', paymentData);
    // Clear cart, show success message, redirect to purchases page, etc.
    onClose();
    
    // Show success message (you can implement a toast notification here)
    alert('Payment successful! Your items are now available in your purchases.');
  };
              ...
              >
                ...
                <div className="d-flex gap-3">
                  ...
                  <button 
                    ...
                    disabled={total <= 0} // Disable if total is zero
                    style={{
                      ...
                      transition: 'all 0.3s ease',
                      opacity: total <= 0 ? 0.6 : 1 // Dim if disabled
                    }}
                    onMouseEnter={(e) => {
                      if (total > 0) {
